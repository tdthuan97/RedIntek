...
def isHase(key):
    if exists(key):
        return isinstance(DataBase[key], dict)
    return False


def isHaseKey(key, hash_key):
    """
    Check is hash_key in key at DataHash
    """
    global DataBase
    if exists(key):
        return hash_key in DataBase[key]
    return False
...
def hset(key, hash_key, hash_value):
    ...
    if exists(key):
        DataBase[key][hash_key] = hash_value
    else:
        DataBase[key] = data
    return data
...
def hget(key, hash_key):
    ...
    """
    if isHase(key):
        return DataBase[key][hash_key] if isHaseKey(key, hash_key) else None
    return None
...
def hgetall(key):
    ...
    """
    return DataBase[key] if isHase(key) else None
